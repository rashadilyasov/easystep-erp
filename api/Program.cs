...
// PostgreSQL: TCP_ABORT_O/D əleyhinə — Railway Hobby ~20 connection limit
if (!useSqlite && !conn.Contains("Connection Idle Lifetime", StringComparison.OrdinalIgnoreCase))
    conn += ";Connection Idle Lifetime=60;Command Timeout=30;Maximum Pool Size=10;Minimum Pool Size=0";
    ...
    try
    {
        if (useSqlite)
        {
            ...
            try
            {
                ...
                try { await db.Database.ExecuteSqlRawAsync("ALTER TABLE Affiliates ADD COLUMN BankIban TEXT"); } catch { }
                try { await db.Database.ExecuteSqlRawAsync("ALTER TABLE Affiliates ADD COLUMN BankName TEXT"); } catch { }
                try { await db.Database.ExecuteSqlRawAsync("ALTER TABLE Affiliates ADD COLUMN BankAccountHolder TEXT"); } catch { }
                try { await db.Database.ExecuteSqlRawAsync("ALTER TABLE Affiliates ADD COLUMN PayriffInfo TEXT"); } catch { }
                try { await db.Database.ExecuteSqlRawAsync("ALTER TABLE Affiliates ADD COLUMN CommissionReceiveMethod INTEGER DEFAULT 0"); } catch { }
                try { await db.Database.ExecuteSqlRawAsync("ALTER TABLE Affiliates ADD COLUMN CommissionAccountNote TEXT"); } catch { }
        ...
        else
        {
            ...
            try { await db.Database.ExecuteSqlRawAsync(@"CREATE INDEX IF NOT EXISTS ""IX_Tickets_CreatedAt"" ON ""Tickets"" (""CreatedAt"")"); } catch { }
            try { await db.Database.ExecuteSqlRawAsync(@"CREATE TABLE IF NOT EXISTS ""TicketAttachments"" (""Id"" uuid NOT NULL, ""TicketId"" uuid NOT NULL, ""FileName"" text NOT NULL, ""ContentType"" text NOT NULL, ""Content"" bytea NOT NULL, ""CreatedAt"" timestamp with time zone NOT NULL, CONSTRAINT ""PK_TicketAttachments"" PRIMARY KEY (""Id""), CONSTRAINT ""FK_TicketAttachments_Tickets_TicketId"" FOREIGN KEY (""TicketId"") REFERENCES ""Tickets"" (""Id"") ON DELETE CASCADE)"); } catch { }
            try { await db.Database.ExecuteSqlRawAsync(@"CREATE INDEX IF NOT EXISTS ""IX_TicketAttachments_TicketId"" ON ""TicketAttachments"" (""TicketId"")"); } catch { }
            try { await db.Database.ExecuteSqlRawAsync(@"CREATE TABLE IF NOT EXISTS ""ContactMessages"" (""Id"" uuid NOT NULL, ""Name"" text NOT NULL, ""Email"" text NOT NULL, ""Message"" text NOT NULL, ""CreatedAt"" timestamp with time zone NOT NULL, CONSTRAINT ""PK_ContactMessages"" PRIMARY KEY (""Id""))"); } catch { }
            try { await db.Database.ExecuteSqlRawAsync(@"CREATE INDEX IF NOT EXISTS ""IX_ContactMessages_CreatedAt"" ON ""ContactMessages"" (""CreatedAt"")"); } catch { }
            try { await db.Database.ExecuteSqlRawAsync(@"CREATE TABLE IF NOT EXISTS ""SiteContents"" (""Id"" uuid NOT NULL, ""Key"" text NOT NULL, ""Value"" text NOT NULL, ""UpdatedAt"" timestamp with time zone NOT NULL, CONSTRAINT ""PK_SiteContents"" PRIMARY KEY (""Id""))"); } catch { }
            try { await db.Database.ExecuteSqlRawAsync(@"CREATE UNIQUE INDEX IF NOT EXISTS ""IX_SiteContents_Key"" ON ""SiteContents"" (""Key"")"); } catch { }
            ...
            try
            {
                ...
                try { await db.Database.ExecuteSqlRawAsync(@"DO $$ BEGIN ALTER TABLE ""Affiliates"" ADD COLUMN ""BankIban"" character varying(50) NULL; EXCEPTION WHEN duplicate_column THEN NULL; END $$"); } catch { }
                try { await db.Database.ExecuteSqlRawAsync(@"DO $$ BEGIN ALTER TABLE ""Affiliates"" ADD COLUMN ""BankName"" character varying(100) NULL; EXCEPTION WHEN duplicate_column THEN NULL; END $$"); } catch { }
                try { await db.Database.ExecuteSqlRawAsync(@"DO $$ BEGIN ALTER TABLE ""Affiliates"" ADD COLUMN ""BankAccountHolder"" character varying(200) NULL; EXCEPTION WHEN duplicate_column THEN NULL; END $$"); } catch { }
                try { await db.Database.ExecuteSqlRawAsync(@"DO $$ BEGIN ALTER TABLE ""Affiliates"" ADD COLUMN ""PayriffInfo"" character varying(200) NULL; EXCEPTION WHEN duplicate_column THEN NULL; END $$"); } catch { }
                try { await db.Database.ExecuteSqlRawAsync(@"DO $$ BEGIN ALTER TABLE ""Affiliates"" ADD COLUMN ""CommissionReceiveMethod"" integer NOT NULL DEFAULT 0; EXCEPTION WHEN duplicate_column THEN NULL; END $$"); } catch { }
                try { await db.Database.ExecuteSqlRawAsync(@"DO $$ BEGIN ALTER TABLE ""Affiliates"" ADD COLUMN ""CommissionAccountNote"" character varying(500) NULL; EXCEPTION WHEN duplicate_column THEN NULL; END $$"); } catch { }
