...
export default function CabinetSecurityPage() {
  ...
  const load2FAStatus = () => {
    api.auth.twoFactorStatus().then((s) => {
      setTwoFactorEnabled(s.twoFactorEnabled);
      setTwoFactorViaEmail(s.twoFactorViaEmail);
    }).catch(() => {
  ...
  return (
    <div>
      ...
      {twoFactorEnabled && (
        <div className="mb-6 px-4 py-3 rounded-xl bg-green-50 text-green-800 border border-green-200">
          <span className="font-medium">2FA aktiv</span> — {twoFactorViaEmail ? "E-poçt OTP" : "Authenticator app"}
        </div>
      )}
        ...
        <div className="p-6 bg-white rounded-2xl border border-slate-200">
          ...
          {!setup ? (
            <div className="flex flex-wrap gap-2">
              <button
                onClick={() => !twoFactorEnabled && handleSetup(false)}
                disabled={loading || twoFactorEnabled}
                className={`px-4 py-2 rounded-lg transition-opacity ${
                  twoFactorEnabled && !twoFactorViaEmail
                    ? "opacity-60 bg-slate-100 text-slate-600 border border-slate-200 cursor-default"
                    : "bg-primary-600 text-white hover:bg-primary-700 disabled:opacity-50"
                }`}
              >
                {twoFactorEnabled && !twoFactorViaEmail ? "Authenticator app ilə ✓" : "Authenticator app ilə"}
              </button>
              <button
                onClick={() => !twoFactorEnabled && handleSetup(true)}
                disabled={loading || twoFactorEnabled}
                className={`px-4 py-2 rounded-lg transition-opacity ${
                  twoFactorEnabled && twoFactorViaEmail
                    ? "opacity-60 bg-slate-100 text-slate-600 border border-slate-200 cursor-default"
              ...
              >
                {twoFactorEnabled && twoFactorViaEmail ? "E-poçt OTP ilə ✓" : "E-poçt OTP ilə"}
              </button>
            </div>
          ) : setup.viaEmail ? (
